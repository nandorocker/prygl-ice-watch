...
const App: React.FC = () => {
  ...
  const handleRefresh = async (force = false) => {
    ...
    try {
      ...
      // Sequence: 1. Start fading out loader
      setTimeout(() => {
        setShowLoadingOverlay(false);
        // Sequence: 2. Reveal content once loader is fully transparent
        setTimeout(() => {
          setRevealContent(true);
        }, 1000);
      }, 800);
    } catch (err: any) {
      setError("UPSTREAM SIGNAL FAILURE");
  ...
  return (
    ...
    >
      ...
      {/* Loading Overlay */}
      <div 
        className={`fixed inset-0 z-[200] flex flex-col items-center justify-center transition-opacity duration-1000 pointer-events-none ${showLoadingOverlay ? 'opacity-100' : 'opacity-0'}`}
      >
        <div className="flex flex-col items-center gap-8">
          <div className="w-24 h-24 md:w-32 md:h-32 border-4 border-[#FDF6E3] rounded-full flex items-center justify-center">
            <IceCubeIcon className="w-12 h-14 md:w-16 md:h-18" spinning={true} />
          </div>
          <div className="flex flex-col items-center">
            <h1 className="font-display text-4xl md:text-6xl leading-none uppercase text-[#FDF6E3]">Is Prygl Frozen?</h1>
            <div className="h-1 w-24 bg-[#FDF6E3] mt-4 opacity-30"></div>
            <span className="font-mono text-[9px] md:text-[10px] tracking-[0.6em] uppercase text-[#FDF6E3] mt-4 opacity-60">Consulting sources...</span>
          </div>
        </div>
      </div>
      ...
      {revealContent && (
        <div className="flex-1 flex flex-col animate-reveal">
          <header className="relative z-10 w-full px-6 md:px-12 pt-6 md:pt-10 flex justify-between items-center shrink-0">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 md:w-14 md:h-14 border-2 border-current rounded-full flex items-center justify-center">
                <IceCubeIcon />
              </div>
              <div className="flex flex-col">
                <h1 className="font-display text-2xl md:text-4xl leading-none uppercase">Is Prygl Frozen?</h1>
                <span className="font-mono text-[8px] md:text-[9px] tracking-[0.3em] opacity-50 uppercase">An Independent Monitor</span>
              </div>
            </div>
          ...
          <main className="relative z-10 flex-1 flex flex-col items-center justify-center py-4">
            ...
            {report && (
              <div className="w-full flex flex-col items-center">
                <StatusIndicator status={report.canSkate} timestamp={report.lastUpdated} statusColor={currentBg} />
          ...
          <footer className="relative z-10 w-full px-6 md:px-12 pb-6 md:pb-10 flex flex-col md:flex-row justify-between items-center gap-4 border-t-2 border-[#FDF6E3]/20 shrink-0">
            <div className="flex flex-col gap-1 max-w-md text-center md:text-left">
              <div className="flex items-center justify-center md:justify-start gap-2">
                ...
                <span className="font-mono text-[9px] font-bold tracking-widest uppercase">Independent Watch Feed</span>
              </div>
              <p className="text-[9px] font-mono leading-relaxed opacity-40 italic uppercase">
                * ICE CONDITIONS ARE ESTIMATES. USE AT YOUR OWN RISK. NOT AN OFFICIAL SOURCE.
              </p>
            </div>
